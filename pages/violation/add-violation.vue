	...
	methods: {
		...
		async callRecognitionAPI(base64Image, isAutoRecognition = false) {
			...
			try {
				...
				const response = await uni.request({
					url: 'https://localhost:8543/api/plate/recognize',
					method: 'POST',
					header: {
		...
		async sendViolationSms(phoneNumber, ownerName, violationId) {
			try {
				...
				// 短信配置（使用动态模板或后备默认值）
				const smsConfig = {
					baseUrl: 'https://localhost:8543', // 使用正确的后端服务地址
		...
		async checkBlacklistThreshold(plateNumber) {
			try {
				...
				const response = await uni.request({
					url: `https://localhost:8543/parking/violations/threshold-remaining/${plateNumber}`,
					method: 'GET',
					data: {
		...
		async loadBlacklistTypes() {
			...
			try {
				...
				const response = await uni.request({
					url: 'https://localhost:8543/parking/acms/vip/blacklist-types',
					method: 'POST',
					header: {
						'Content-Type': 'application/json'
					},
