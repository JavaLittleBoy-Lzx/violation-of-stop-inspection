		...
		<view v-for="(item, index) in values" :class="[styleType === 'text' ? '' : 'segmented-control__item--button',
			...
			<view>
				<text
					...
					:class="styleType === 'text' && index === currentIndex ? 'segmented-control__item--text': ''">{{ item.title||item }}</text>
			</view>
	...
	export default {
		...
		computed: {
			...
			computedBackgroundStyle() {
				return index => {
					return {
						backgroundColor: index === this.currentIndex && this.styleType === 'button' ? this
							.activeColor : this
							.styleType === 'button' ? this.inActiveColor : 'transparent',
						borderColor: index === this.currentIndex && this.styleType === 'text' || this.styleType ===
							'button' ? this
