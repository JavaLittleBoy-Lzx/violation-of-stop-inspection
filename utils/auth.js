...
let API_BASE_URL = 'https://localhost:8543/api/auth';
      ...
      try {
        ...
        if (yardId) {
          ...
          smsResponse = await uni.request({
            url: `https://localhost:8543/parking/yardInfo/${yardId}/sms-templates`,
            method: 'GET',
            header: {
          ...
          //  Fallback: 使用车场名称查询
          smsResponse = await uni.request({
            url: `https://localhost:8543/parking/yardInfo/sms-templates/by-name?parkName=${encodeURIComponent(parkName)}`,
            method: 'GET',
            header: {
              'Authorization': `Bearer ${token}`,
              'Content-Type': 'application/json'
